...
async function fetchMovieDetails(movieId) {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=b8da134575df7f366bd677a5e1daf01e&append_to_response=credits`);
...
async function fetchMovieTrailer(movieId) {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=b8da134575df7f366bd677a5e1daf01e`);
        const data = await response.json();
...
async function getCastHTML(cast) {
    const castHTMLPromises = cast.slice(0, 10).map(async (castMember) => {
        const response = await fetch(`https://api.themoviedb.org/3/person/${castMember.id}?api_key=b8da134575df7f366bd677a5e1daf01e`);
        const data = await response.json();
